...
class MainWindow(QMainWindow):
    ...
    def show_results(self, analysis: dict, result_type: str = "text"):
        ...
        if result_type == "text" or result_type == "parse":
            # Оценки: дизайн и доверие к свежести
            if "design_score" in analysis or "freshness_trust_score" in analysis:
                scores_frame = QFrame()
                scores_frame.setObjectName("resultBlock")
                scores_layout = QHBoxLayout(scores_frame)

                for label_text, key in (("🎨 Дизайн", "design_score"), ("🐟 Доверие к свежести", "freshness_trust_score")):
                    if key in analysis:
                        box = QVBoxLayout()
                        score_title = QLabel(label_text)
                        score_title.setObjectName("sectionTitle")
                        score_value = QLabel(f"{analysis[key]}/10")
                        score_value.setStyleSheet("font-size: 28px; font-weight: bold; color: #22d3ee;")
                        box.addWidget(score_title)
                        box.addWidget(score_value)
                        scores_layout.addLayout(box)

                self.results_layout.addWidget(scores_frame)
            ...
            # Оценка доверия к свежести
            if "freshness_trust_score" in analysis:
                trust_frame = QFrame()
                trust_frame.setObjectName("resultBlock")
                trust_layout = QVBoxLayout(trust_frame)

                trust_title = QLabel("🐟 Доверие к свежести")
                trust_title.setObjectName("sectionTitle")

                trust_value = QLabel(f"{analysis['freshness_trust_score']}/10")
                trust_value.setStyleSheet("font-size: 32px; font-weight: bold; color: #22d3ee;")

                trust_layout.addWidget(trust_title)
                trust_layout.addWidget(trust_value)
                self.results_layout.addWidget(trust_frame)
