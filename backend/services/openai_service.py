...
class OpenAIService:
    ...
    async def analyze_text(self, text: str) -> CompetitorAnalysis:
        ...
        system_prompt = """Ты — эксперт по конкурентному анализу в нише интернет-магазинов морепродуктов
(камчатский краб, креветки, гребешки, икра, доставка на дом). Проанализируй предоставленный текст
конкурента и верни структурированный JSON-ответ.
...
Формат ответа (строго JSON):
{
    ...
    "design_score": 7,
    "freshness_trust_score": 8
}

Важно:
- Каждый массив должен содержать 3-5 пунктов
- design_score (0-10): насколько текст и подача материала выглядят стильно и профессионально
- freshness_trust_score (0-10): насколько текст внушает доверие к свежести, происхождению
  и качеству морепродуктов (упоминания вылова/заморозки, сроков доставки, сертификатов и т.п.)
...
- Будь конкретен и практичен в рекомендациях"""
        ...
        try:
            ...
            result = CompetitorAnalysis(
                ...
                freshness_trust_score=data.get("freshness_trust_score", 0)
            )

            logger.info(f"  Результат: {len(result.strengths)} сильных, {len(result.weaknesses)} слабых сторон")
            logger.info(f"  design_score={result.design_score}/10, freshness_trust_score={result.freshness_trust_score}/10")
            logger.info("=" * 50)
...
морепродуктов (камчатский краб, креветки, гребешки, икра). Проанализируй изображение конкурента
(баннер, скриншот сайта, фото товара и т.д.) и верни структурированный JSON-ответ.
...
Важно:
- visual_style_score и design_score от 0 до 10 (можно ставить одинаковое значение — обе оценки
  про визуальный стиль/UX)
- freshness_trust_score (0-10): насколько кадр/фото/сайт визуально внушает доверие к свежести,
  премиальности и качеству морепродуктов (естественный цвет продукта, лёд, упаковка, чистота кадра)
- Каждый массив должен содержать 3-5 пунктов
- Пиши на русском языке
- Оценивай: цветовую палитру, типографику, композицию, UX/UI элементы"""
        ...
        try:
            ...
            result = ImageAnalysis(
                ...
                recommendations=data.get("recommendations", []),
                design_score=data.get("design_score", data.get("visual_style_score", 0)),
                freshness_trust_score=data.get("freshness_trust_score", 0)
            )
    ...
    async def analyze_website_screenshot(
        ...
        system_prompt = """Ты — эксперт по конкурентному анализу и UX/UI дизайну в нише интернет-магазинов
морепродуктов (камчатский краб, креветки, гребешки, икра, доставка на дом). Проанализируй скриншот
сайта конкурента и верни структурированный JSON-ответ.
...
- Насколько сайт внушает доверие к свежести и происхождению морепродуктов (фото, сертификаты,
  упоминания вылова/заморозки/сроков доставки) — это ляжет в freshness_trust_score

Важно:
- Каждый массив должен содержать 4-6 конкретных пунктов
- design_score (0-10): визуальный стиль/UX сайта в целом
- freshness_trust_score (0-10): доверие к свежести и качеству продукта, которое внушает сайт
- Пиши на русском языке
