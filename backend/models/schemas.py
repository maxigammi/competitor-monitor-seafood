...
class CompetitorAnalysis(BaseModel):
    ...
    design_score: int = Field(0, ge=0, le=10, description="Оценка визуального стиля/UX сайта (0-10)")
    freshness_trust_score: int = Field(
        0, ge=0, le=10,
        description="Насколько текст/сайт внушает доверие к свежести и качеству морепродуктов (0-10)"
    )
...
class ImageAnalysis(BaseModel):
    ...
    recommendations: List[str] = Field(default_factory=list, description="Рекомендации")
    design_score: int = Field(0, ge=0, le=10, description="Оценка визуального стиля/UX (0-10)")
    freshness_trust_score: int = Field(
        0, ge=0, le=10,
        description="Насколько изображение внушает доверие к свежести и премиальности морепродуктов (0-10)"
    )
